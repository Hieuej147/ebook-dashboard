...
export default function DialogBook() {
  ...
  const chapters = agent.state?.book?.chapters || [];
  ...
  const handleCreateBook = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      const payload = {
        title: agent.state.book?.title,
        subtitle: agent.state.book?.topic,
        author: authorName || agent.state.book.author, // Dùng biến từ state
        description: `A ${agent.state.book?.writingStyle} book about ${agent.state.book?.topic}`,
        price: 19.99,
        stock: 1,
        sku: `BK-${Date.now()}`,
        categoryId: "0aab6a42-03e9-4cab-a95a-e72e56043e49",
      };
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Field>
                      ...
                      <Input
                        ...
                        name="chapters"
                        type="number"
                        defaultValue={agent.state?.book?.chaptersCount || 3}
                        min={1}
                      />
                    </Field>
                    <Field>
                ...
                <DialogFooter className="flex flex-row items-center justify-between sm:justify-between w-full pt-2 border-t">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      disabled={agent.isRunning || chapters.length === 0}
                    >
                      {isSaving ? (
                        <>
