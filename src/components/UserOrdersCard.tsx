...
const UserOrdersCard = ({ userId }: UserOrdersCardProps) => {
  ...
  useEffect(() => {
    const fetchUserOrders = async () => {
      ...
      try {
        const res = await fetch(`/api/orders/user/${userId}?${params.toString()}`);
        const result = await res.json();

        // Bóc tách data thật từ cái bọc của NestJS
        const cleanOrders = result.data?.map((wrapper: any) => wrapper.data) || [];
  ...
  return (
    <div className="border shadow-sm p-4 rounded-2xl bg-white h-full">
        ...
        <div className="flex flex-col gap-3">
          {loading ? (
            ...
            orders.map((order: any) => (
              <Card
                key={order.id}
                className="p-3 hover:bg-slate-50 transition-all group border-slate-100 cursor-pointer hover:border-purple-200"
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="bg-white size-8 rounded-full flex items-center justify-center border shadow-sm opacity-0 group-hover:opacity-100 transition-all transform group-hover:scale-110">
